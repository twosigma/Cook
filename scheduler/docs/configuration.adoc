...
`:plugins`:: Cook has two extension points that let plugins reject jobs at job submission time as well as accept or defer the launching of jobs via `:job-launch-filter` and `:job-submission-valiator`.
...
* `:job-submission-valiator`  configures the entrypoint for filtering job submissions. It has several keys:
