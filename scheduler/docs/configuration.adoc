 ...
 * `:factory-fn` contains a string with a namespace-qualified path of a function for creating either a `JobLaunchFilter` or `JobSubmissionValidator`

 * `:arguments` contains a dictionary that is passed to the factory-fn to initialize the hook object.
