...
`:job-launch-filter` and `:job-submission-valiator`::  Cook has two extension points that let plugins reject jobs at job submission time as well as accept or defer the launching of jobs. The plugins for controlling job launch must implment the `JobLaunchFilter` and be specified under `:job-launch-filter`. The plugin for controlling job submission must implement the `JobSubmissionValidator` protocol and be specified under `:job-submission-valiator`. The value for these keys is a dictionary with some configuration settings as well as:

 * `:factory-fn` contains a string with a namespace-qualified path of a function for creating either a `JobLaunchFilter` or `JobSubmissionValidator`. The factory function can fetch the current configuration out of the config defstate in `cook.config/config`
