...
`:launch-hook` and `:submission-hook`::  Cook has two hooks that let plugins reject jobs at job submission time as well as accept or defer the launching of jobs. The plugins for controlling job launch must implment the `JobLaunchFilter` and be specified under `:launch-hook`. THe plugin for controlling job submission must implement the `JobSubmissionValidator` protocol and be specified under `:submission-hook`. The value for these keys is a dictionary with some configuration settings as well as:
