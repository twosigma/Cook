...
`:hook-factory`::  Cook hooks is a plugin API that lets job submission and job launch be controlled via a plugin. The plugin can reject jobs at job submission time as well as accept or defer the launching of jobs. The plugin must implment the SchedulerHooks protocol. The value for this key is a dictionary with two entries:

 * `:factory-fn` contains a string with a namespace-qualified path of a function for creating a hook object.
