       ...
       (try
         ...
         (loop [state {:config {:task->runtime-ms task->runtime-ms
           ...
           (let [;start-ms (System/currentTimeMillis)
                 [v ch] (async/alts!! [exit-chan action-chan offer-trigger-chan complete-trigger-chan]
                                      :priority true)
                 ;wait-ms (System/currentTimeMillis)
             ;    _ (log/debug {:ms-waited-for-ch (- wait-ms start-ms)})
             ...
             ;(log/debug {:ms-to-process (- (System/currentTimeMillis) wait-ms)})
             (when state'
               (recur state'))))
         (catch Exception ex
