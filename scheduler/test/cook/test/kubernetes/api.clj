...
(deftest test-dissoc-in
  (is (= {:a {:c 3} :d {:e 5 :f 6}} (api/dissoc-in {:a {:b 2 :c 3} :d {:e 5 :f 6}} [:a :b])))
  (is (= {:a {:b 2} :d {:e 5 :f 6}} (api/dissoc-in {:a {:b 2 :c 3} :d {:e 5 :f 6}} [:a :c])))
  (is (= {:a {:b 2 :c 3} :d {:e 5}} (api/dissoc-in {:a {:b 2 :c 3} :d {:e 5 :f 6}} [:d :f])))
  ; These keys aren't in it.
  (is (= {:a {:b 2 :c 3} :d {:e 5 :f 6}} (api/dissoc-in {:a {:b 2 :c 3} :d {:e 5 :f 6}} [:a :d])))
  (is (= {:a {:b 2 :c 3} :d {:e 5 :f 6}} (api/dissoc-in {:a {:b 2 :c 3} :d {:e 5 :f 6}} [:b :c])))

  ;; Now try deleting from smaller dictionaries and make sure it deletes empty leafs.
  (is (= {:d {:e 5 :f 6}} (api/dissoc-in {:a {:b 2} :d {:e 5 :f 6}} [:a :b])))
  (is (= {:a {:b 2}} (api/dissoc-in {:a {:b 2} :d {:e 5}} [:d :e])))
  (is (= {} (api/dissoc-in {:a {:b 2}} [:a :b])))

  ;; Check for nil safety.
  (is (= {:a {:b 2 :c 3} nil {nil 6}} (api/dissoc-in {:a {:b 2 :c 3} nil {:e 5 nil 6}} [nil :e])))
  (is (= {:a {:b 2 :c 3} nil {:e 5}} (api/dissoc-in {:a {:b 2 :c 3} nil {:e 5 nil 6}} [nil nil])))
  (is (= {:a {:b 2} nil {:e 5 nil 6}} (api/dissoc-in {:a {:b 2 nil 3} nil {:e 5 nil 6}} [:a nil])))
  (is (= {:a {nil 3} nil {:e 5 nil 6}} (api/dissoc-in {:a {:b 2 nil 3} nil {:e 5 nil 6}} [:a :b])))

  (is (= {:a {:b 2} :d {:e 5 :f 6}} (api/dissoc-in {:a {:b 2} :d {:e 5 :f 6}} [nil :b])))
  (is (= {:a {:b 2} :d {:e 5 :f 6}} (api/dissoc-in {:a {:b 2} :d {:e 5 :f 6}} [:nil nil])))
  (is (= {:d {:e 5 :f 6}} (api/dissoc-in {nil {:b 2} :d {:e 5 :f 6}} [nil :b])))
  (is (= {:a {:b 2}} (api/dissoc-in {:a {:b 2} :d {nil 5}} [:d nil]))))

(deftest test-make-nested-atom-updater
  (let [map-atom (atom {})
        testfn (api/make-nested-atom-updater map-atom :k1 :k2)]
    (testing "Inserting"
      (let [d1 {:k1 :a :k2 :b}
            _ (testfn nil nil d1)
