...
class ProgressWatcher(object):
    ...
    def tail(self, sleep_time_ms):
        ...
        :param sleep_time_ms: The unit of time in ms to repetitively sleep when the file has not been created or no new
    ...
    def retrieve_progress_states(self):
        """
        It tails a target file (using the tail() method) and uses the provided regex to find a match for a progress
        message. The regex is expected to generate two components in the match: the progress percent as an int and a
        progress message string. When such a message is found, this method yields the current progress as a dictionary.
...
def track_progress(driver, task_id, progress_watcher, max_message_length, progress_complete_event,
    ...
    """
        ...
        :param progress_watcher: The progress watcher which maintains the current progress state
        :param max_message_length: Allowed max message length to send
        :param progress_complete_event: Event that triggers completion of progress tracking
        :param send_progress_update: The function to invoke while sending progress updates
        :return: Nothing.
    """
    try:
